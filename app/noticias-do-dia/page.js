...
export default function NoticiasDoDiaPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/posts?type=noticia", { cache: "no-store" })
      .then((res) => res.json())
      .then((data) => {
        if (!ativo) return;
    ...
    <main style={styles.page}>
      <div style={styles.container}>
        <div style={styles.header}>
          <div>
            <h1 style={styles.title}>Notícias do Dia</h1>
            <p style={styles.subtitle}>
              As principais notícias publicadas no BolaNoBrasil.
            </p>
          </div>
          ...
          <div style={styles.list}>
            {posts.map((item) => (
              <article key={item.id || item.slug} style={styles.card}>
                <span style={styles.badge}>Notícias do Dia</span>
