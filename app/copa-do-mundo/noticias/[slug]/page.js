...
export default function NoticiaCopaInternaPage({ params }) {
  ...
  async function loadItem() {
    try {
      const res = await fetch(
        `/api/copa-do-mundo/noticias/${params.slug}?t=${Date.now()}`,
        { cache: "no-store" }
      );
  ...
  if (!item) {
    return (
      <main style={{ minHeight: "100vh", background: "#071024", color: "#f5f7fa", padding: 16 }}>
        <Link href="/copa-do-mundo/noticias" style={{ color: "#f5f7fa" }}>
          Voltar às notícias da Copa
        </Link>
