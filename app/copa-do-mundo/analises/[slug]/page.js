...
export default function AnaliseCopaInternaPage({ params }) {
  const [item, setItem] = useState(null);
  const [loading, setLoading] = useState(true);

  async function loadItem() {
    try {
      const res = await fetch(`/api/copa-do-mundo/analises/${params.slug}?t=${Date.now()}`, { cache: "no-store" });
  ...
  if (loading) return <main style={{ minHeight: "100vh", background: "#071024", color: "#f5f7fa", padding: 16 }}>Carregando análise...</main>;

  if (!item) {
    return (
      <main style={{ minHeight: "100vh", background: "#071024", color: "#f5f7fa", padding: 16 }}>
        <Link href="/copa-do-mundo/analises" style={{ color: "#f5f7fa" }}>Voltar às análises da Copa</Link>
  ...
  return (
    <main style={{ minHeight: "100vh", background: "radial-gradient(circle at top, rgba(19,44,99,0.28) 0%, #071024 34%, #03060d 100%)", color: "#f5f7fa" }}>
      <section style={{ width: "100%", maxWidth: 860, margin: "0 auto", padding: "18px 16px 48px" }}>
        <Link href="/copa-do-mundo/analises" style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", minHeight: 42, padding: "0 14px", borderRadius: 14, textDecoration: "none", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.10)", color: "#f5f7fa", fontWeight: 800, marginBottom: 16 }}>
          Voltar às análises da Copa
        </Link>
