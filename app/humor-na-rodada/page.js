...
const styles = {
  ...
  button: {
    display: "inline-block",
    textDecoration: "none",
    background: "linear-gradient(135deg, #d89b57, #b9762c)",
    color: "#fff",
    fontWeight: 800,
    padding: "10px 16px",
    borderRadius: 12,
...
export default function HumorNaRodadaPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/posts?type=humor", { cache: "no-store" })
      .then((res) => res.json())
      .then((data) => {
        if (!ativo) return;
    ...
    <main style={styles.page}>
      <div style={styles.container}>
        <div style={styles.header}>
          <div>
            <h1 style={styles.title}>Humor na Rodada</h1>
            <p style={styles.subtitle}>
              Os textos de humor publicados no BolaNoBrasil.
            </p>
          </div>
        ...
        {loading ? (
          <p style={styles.loading}>Carregando textos...</p>
        ) : (
            ...
            {posts.map((item) => (
              <article key={item.id || item.slug} style={styles.card}>
                ...
                <div style={styles.meta}>
                  ...
                  <Link
                    href={`/humor-na-rodada/${item.slug}`}
                    style={styles.button}
                  >
                    Ler texto
                  </Link>
