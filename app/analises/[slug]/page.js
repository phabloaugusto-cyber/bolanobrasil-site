...
export default function AnaliseSlugPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/posts/${slug}`, { cache: "no-store" })
      ...
      .then((data) => {
        if (!ativo) return;
        if (data && !data.error && data.type === "analise") {
          setPost(data);
        } else {
          setPost(null);
        }
    ...
    <main style={styles.page}>
      <div style={styles.container}>
        <div style={styles.top}>
          <Link href="/analises" style={styles.backLink}>
        ...
        {loading ? (
          <p style={styles.loading}>Carregando análise...</p>
        ) : !post ? (
          <p style={styles.loading}>Análise não encontrada.</p>
        ) : (
          <>
